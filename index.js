...
async function loadM(mr,mc){
  ...
  favico(j.theme.w);{let _mm=document.getElementById('M');if(_mm)_mm.classList.toggle('pool',!!(j.theme&&j.theme.grid))};{var _pg=j&&j.theme&&j.theme.grid;document.body.classList.toggle('poolbody',!!_pg);if(_pg){if(!window.__poolOv){var ov=document.createElement('div');ov.id='poolOv';document.body.appendChild(ov);window.__poolOv=ov;var draw=function(){var M=document.getElementById('M');var ii=document.querySelector('i');if(!M||!document.body.classList.contains('poolbody')){ov.style.display='none';window.__poolRAF=requestAnimationFrame(draw);return;}ov.style.display='block';var mr=M.getBoundingClientRect();var sz=parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--size'));var h=sz/2;ov.style.cssText='position:fixed;pointer-events:none;display:block;z-index:49;left:'+mr.left+'px;top:'+mr.top+'px;width:'+mr.width+'px;height:'+mr.height+'px';ov.style.background='linear-gradient(to right,rgba(8,60,54,0.85) 2px,transparent 2px) 0 0/'+h+'px '+h+'px,linear-gradient(to bottom,rgba(8,60,54,0.85) 2px,transparent 2px) 0 0/'+h+'px '+h+'px,#5fc0ac';if(ii){var ir=ii.getBoundingClientRect();var cx=ir.left+ir.width/2-mr.left;var cy=ir.top+ir.height/2-mr.top;var rad=sz*2.4;var m='radial-gradient(circle '+rad+'px at '+cx+'px '+cy+'px,rgba(0,0,0,1) 0%,rgba(0,0,0,0.97) 25%,rgba(0,0,0,0.8) 42%,rgba(0,0,0,0.5) 60%,rgba(0,0,0,0.22) 76%,rgba(0,0,0,0.07) 90%,transparent 100%)';ov.style.webkitMaskImage=m;ov.style.maskImage=m;}window.__poolRAF=requestAnimationFrame(draw);};window.__poolRAF=requestAnimationFrame(draw);}}else if(window.__poolOv){window.__poolOv.style.display='none';}}
